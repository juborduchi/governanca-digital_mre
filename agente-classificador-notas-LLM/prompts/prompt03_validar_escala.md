# prompt03_validar_escala.md — Validação da Escala Ordinal por LLM

> **Posição no pipeline:** Etapa 4 — após a atribuição da escala ordinal (prompt01_llm.md).
...
e com o conteúdo efetivo dos `paragrafos`.

## Princípios (OBRIGATÓRIOS)
- **Leitura real dos `paragrafos`**: para cada nota, leia o campo **`paragrafos`** do
  JSON de escala ordinal. Não julgue por título ou resumo.
...
3. Verifique se a **justificativa** original está alinhada com a nota e com os parâmetros.
4. Decida: **Coerente** (classificação correta) ou **Inconsistente** (possível erro).
5. Para cada nota inconsistente, registre:
   - **Motivo** da inconsistência
   - **Nota sugerida** (se aplicável)
   - **Justificativa da alteração** (se aplicável)
6. Escreva sua **própria justificativa** para a validação (2–3 frases).
...
- **JSON** `validacao-escala-[modelo]-[data].json` — com:
  - `metadata` (modelo, versão, data, arquivo de escala ordinal original, contexto01.md)
  ...
  - `notas_coerentes`: lista das notas **mantidas**, contendo `titulo`, `data`, `link`,
    ...
    `passagens_relevantes` e `paragrafos`
...
- **CSV** `validacao-escala-[modelo]-[data].csv` — planilha com todas as notas avaliadas,
  ...
  `justificativa_validacao`. Separador: `;`, encoding UTF-8 com BOM para
  compatibilidade com Excel em PT-BR.
...
- Principais padrões de inconsistência identificados.

## Reprodutibilidade
- `temperature=0`; nome do modelo+versão e data nos arquivos.
- Mantenha o JSON de entrada (escala ordinal) e o JSON de saída (validação) para auditoria.
