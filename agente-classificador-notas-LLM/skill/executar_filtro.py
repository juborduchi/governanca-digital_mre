...
# -*- coding: utf-8 -*-
"""
Execucao da FILTRAGEM HEURISTICA de notas a Imprensa do MRE.
Tema (contexto01.md): Governanca Global Digital e tudo que a envolve
(internet, IA, dados e demais tecnologias digitais).

Abordagem: pre-selecao por vocabulario substantivo de governanca digital
e, em seguida, criterio qualitativo (exige conexao direta com o tema no
corpo do texto, descarta mencoes superficiais e meros calendarios/agendas)
e geracao de justificativas em linguagem natural referenciando o conteudo.

A AVALIACAO ORDINAL (nota 1-5) NAO e feita aqui: ela e semantica e deve
ser executada pelo LLM seguindo prompts/prompt01_llm.md, que le o JSON
filtrado gerado por este script.
...
TEMAS = {
    ...
    "Economia e Comercio Digital": [
        "economia digital", "economia digitale", "digital economy",
        "comércio digital", "comercio digital", "digital trade", "digital commerce",
        "comércio eletrônico", "comercio eletronico", "e-commerce", "eletronic commerce",
        "facilitação do comércio eletrônico", "facilitacao do comercio eletronico",
        "plataformas digitais", "plataforma digital", "economia de plataforma",
    ],
...
# Termos genericos que sozinhos nao caracterizam governanca digital (ruido)
...
def gerar_justificativa(titulo, temas, passagens):
    ...
    base = (
        f"A nota guarda relação direta e substantiva com a Governança Global Digital, "
    ...
    if passagens:
        base += (
            ...
            f"governança digital no período coberto pelos dados."
        )
    else:
        base += (
...
def main():
    ...
    fim = int(sys.argv[2]) if len(sys.argv) > 2 else 9999

    arquivos = sorted(JSON_NOTAS_DIR.glob("*.json"))
    todas = []
    analisadas = 0
    descartadas = 0
    ...
    for arq in arquivos:
        ...
        for chave, nota in inner.items():
            ano = ano_de(nota.get("data", ""))
            try:
                if ano is None or not (ini <= int(ano) <= fim):
                    continue
    ...
    print(f"Anos: {ini}-{fim}")
    print(f"Notas analisadas: {analisadas}")
